        ...
        describe('Requesting a valid version (:version = 1.1.0)', () => {
            describe('No query params', () => {
                ...
                describe('Library version object', () => {
                    it("is an object with 'name', 'version', 'files', 'rawFiles' and 'sri' properties", async () => {
                        ...
                        expect(body)
                            ...
                            .that.is.an('array');
                        expect(body)
                            .to.have.property('rawFiles')
                            .that.is.an('array');
                        expect(body)
                            .to.have.property('sri')
                            .that.is.an('object');
                    });
                ...
                it('responds to requests with a trailing slash', async () => {
                    ...
                    expect(
                        await res.json<LibraryVersionResponse>(),
                    ).to.deep.equal(
                        await response.json<LibraryVersionResponse>(),
                    );
                });
            ...
            describe('Requesting a field (?fields=files)', () => {
                ...
                describe('Library version object', () => {
                    it("is an object with only the 'files' property", async () => {
                        expect(await response.json<LibraryVersionResponse>())
                            .to.have.property('files')
                            .that.is.an('array');
                    });
                    it('has no other properties', async () => {
    ...
    describe('Requesting a valid library (:library = backbone.js)', () => {
        describe('No query params', () => {
            ...
            it('returns a JSON body that is a library object', async () => {
                ...
                expect(await response.json<LibraryResponse>()).to.be.an(
                    'object',
                );
            });
            ...
            it('responds to requests with a trailing slash', async () => {
                const res = await request(path + '/');
                expect(res.status).to.eq(200);
                expect(await res.json<LibraryResponse>()).to.deep.equal(
        ...
        describe('Requesting a field (?fields=assets)', () => {
            ...
            describe('Library object', () => {
                it("is an object with only the 'assets' property", async () => {
                    expect(await response.json<LibraryResponse>())
                        .to.have.property('assets')
                        .that.is.an('array');
                });
                it('has no other properties', async () => {
                    expect(
                        Object.keys(await response.json<LibraryResponse>()),
                    ).to.have.lengthOf(1);
                });
            });
        });

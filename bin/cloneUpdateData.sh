...
# Handle cloning or updating the SRI data
(
    exists=$(if [ -d "./data/sri" ]; then echo 0; else echo 1; fi)
    isGit=$(if [ -d "./data/sri" ]; then (cd ./data/sri && git rev-parse --is-inside-work-tree >/dev/null 2>&1 && echo 0) || echo 1; else echo 1; fi)
